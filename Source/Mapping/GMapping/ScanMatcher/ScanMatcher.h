...
namespace NS_GMapping
{
  ...
  inline double
  ScanMatcher::icpStep (OrientedPoint & pret, const ScanMatcherMap& map,
                        const OrientedPoint& p, const double* readings) const
  {
  ...
  inline double
  ScanMatcher::score (const ScanMatcherMap& map, const OrientedPoint& p,
                      const double* readings) const
  {
    double s = 0;
    const double * angle = m_laserAngles + m_initialBeamsSkip;
      ...
      for (int xx = -m_kernelSize; xx <= m_kernelSize; xx++)
        for (int yy = -m_kernelSize; yy <= m_kernelSize; yy++)
        {
              ...
              && ((double) fcell) < m_fullnessThreshold)
          {
            ...
            else bestMu = (mu * mu) < (bestMu * bestMu) ? mu : bestMu;
          }
          //}
        }
      if (found)
  ...
  inline unsigned int
  ScanMatcher::likelihoodAndScore (double& s, double& l,
