...
# endif
  ...
  void
  GridSlamProcessor::setMatchingParameters (double urange, double range,
                                            ...
                                            unsigned int likelihoodSkip)
  {
    ...
    if (m_infoStream)
      m_infoStream << " -maxUrange " << urange << " -maxUrange " << range
          ...
          << " -lstep " << lopt << " -lobsGain " << m_obsSigmaGain << " -astep "
          << aopt << endl;
    
  }
  
  void
  GridSlamProcessor::setMotionModelParameters (double srr, double srt,
  ...
  void
  GridSlamProcessor::setUpdateDistances (double linear, double angular,
                                         double resampleThreshold)
  {
    ...
    if (m_infoStream)
      m_infoStream << " -linearUpdate " << linear << " -angularUpdate "
          << angular << " -resampleThreshold " << m_resampleThreshold << endl;
  }
  ...
  void
  GridSlamProcessor::setSensorMap (const SensorMap& smap)
  {
  ...
  void
  GridSlamProcessor::init (unsigned int size, double xmin, double ymin,
  ...
  void
  GridSlamProcessor::processTruePos (const OdometryReading& o)
  {
  ...
  int
  GridSlamProcessor::getBestParticleIndex () const
  {
    unsigned int bi = 0;
