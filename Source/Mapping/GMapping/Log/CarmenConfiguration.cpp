...
namespace NS_GMapping
{
  ...
  CarmenConfiguration::load (istream& is)
  {
    ...
    while (is)
    {
      ...
      if (qualifier == "ROBOTLASER1")
      {
        ...
        lis >> laser_type >> start_angle >> field_of_view >> angular_resolution
            >> maximum_range >> accuracy >> remission_mode >> beams;
        rlaser1 = true;
      }
      if (qualifier == "ROBOTLASER2")
      {
        string laser_type, start_angle, field_of_view, angular_resolution,
            maximum_range, accuracy, remission_mode;
        lis >> laser_type >> start_angle >> field_of_view >> angular_resolution
            >> maximum_range >> accuracy >> remission_mode >> rbeams;
        rlaser2 = true;
      }
      if (qualifier != "PARAM")
        continue;
  ...
  SensorMap
  CarmenConfiguration::computeSensorMap () const
  {
