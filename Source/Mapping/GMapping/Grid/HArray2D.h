...
namespace NS_GMapping
{
  ...
  template<class Cell>
    ...
    public:
      ...
      inline bool
      isAllocated (const IntPoint& p) const
      {
        return isAllocated (p.x, p.y);
      }
      inline AccessibilityState
      cellState (const IntPoint& p) const
      {
        return cellState (p.x, p.y);
      }
      inline IntPoint
      patchIndexes (const IntPoint& p) const
      {
  ...
  template<class Cell>
    void
    HierarchicalArray2D<Cell>::resize (int xmin, int ymin, int xmax, int ymax)
    {
      int xsize = xmax - xmin;
      int ysize = ymax - ymin;
...
//	Array2D<autoptr< Array2D<Cell> > >::operator=(hg);
  ...
  template<class Cell>
    void
    HierarchicalArray2D<Cell>::setActiveArea (
  ...
  template<class Cell>
    AccessibilityState
    HierarchicalArray2D<Cell>::cellState (int x, int y) const
    {
  ...
  template<class Cell>
    void
    HierarchicalArray2D<Cell>::allocActiveArea ()
    {
  ...
  template<class Cell>
    bool
    HierarchicalArray2D<Cell>::isAllocated (int x, int y) const
    {
  ...
  template<class Cell>
    IntPoint
    HierarchicalArray2D<Cell>::patchIndexes (int x, int y) const
    {
      if (x >= 0 && y >= 0)
