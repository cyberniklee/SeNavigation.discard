...
template<class OutputIterator, class Iterator>
  double
  toNormalForm (OutputIterator& out, const Iterator & begin,
                const Iterator & end)
  {
...
template<class OutputIterator, class Iterator, class Numeric>
  void
  toLogForm (OutputIterator& out, const Iterator & begin, const Iterator & end,
             Numeric lmax)
  {
    //determine the maximum
    for (Iterator it = begin; it != end; it++)
...
template<class WeightVector>
  void
  resample (std::vector<int>& indexes, const WeightVector& weights,
...
template<typename Vector>
  void
  repeatIndexes (Vector& dest, const std::vector<int>& indexes,
...
template<class Iterator>
  double
  neff (const Iterator& begin, const Iterator& end)
  {
    double sum = 0;
    for (Iterator it = begin; it != end; ++it)
    {
...
template<class Iterator>
  void
  normalize (const Iterator& begin, const Iterator& end)
  {
    double sum = 0;
    for (Iterator it = begin; it != end; ++it)
    {
...
template<class OutputIterator, class Iterator>
  void
  rle (OutputIterator& out, const Iterator & begin, const Iterator & end)
  {
...
template<class Particle, class Numeric>
  struct uniform_resampler
  {
    ...
    std::vector<Particle>
    resample (const std::vector<Particle> & particles,
              int nparticles = 0) const;
    Numeric
...
template<class Particle, class Numeric>
  std::vector<unsigned int>
  uniform_resampler<Particle, Numeric>::resampleIndexes (
...
template<class Particle, class Numeric>
  std::vector<Particle>
  uniform_resampler<Particle, Numeric>::resample (
...
template<class Particle, class Numeric>
  Numeric
  uniform_resampler<Particle, Numeric>::neff (
...
template<class Particle, class EvolutionModel>
  void
  evolver<Particle, EvolutionModel>::evolve (std::vector<Particle>& particles)
  {
...
template<class Particle, class EvolutionModel>
  void
  evolver<Particle, EvolutionModel>::evolve (std::vector<Particle>& dest,
...
template<class Particle, class Numeric, class QualificationModel,
    class EvolutionModel, class LikelyhoodModel>
  void
  auxiliary_evolver<Particle, Numeric, QualificationModel, EvolutionModel,
