...
namespace NS_GMapping
{
  ...
  public:
    ...
    double
    integral (double step, double xTo)
    {
      double sum = 0;
    ...
    void
    sampleMultiple (std::vector<double>& samples, int num)
    {
    ...
    void
    approxGauss (double step, double* mean, double* sigma)
    {
    ...
    double
    cramerVonMisesToGauss (double step, double mean, double sigma)
    {
    ...
    double
    kldToGauss (double step, double mean, double sigma)
    {
    ...
    void
    gnuplotDumpSmoothedData (FILE* fp, double step)
    {
      for (double x = m_from; x <= m_to; x += step)
        fprintf (fp, "%f %f\n", x, smoothedData (x));
    }
