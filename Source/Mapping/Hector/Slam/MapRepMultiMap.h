  ...
  public:
                    ...
                    HectorDebugInfoInterface* debugInterfaceIn)
    {
      ...
      for (unsigned int i = 0; i < numDepth; ++i)
      {
        std::cout << "HectorSM map lvl " << i << ": cellLength: "
            << mapResolution << " res x:" << resolution.x () << " res y: "
            << resolution.y () << "\n";
        ...
        ScanMatcher<OccGridMapUtilConfig<GridMap> >* scanMatcher =
            new ScanMatcher<OccGridMapUtilConfig<GridMap> > (drawInterfaceIn,
                                                             debugInterfaceIn);
               ...
               const DataContainer& dataContainer, Eigen::Matrix3f& covMatrix)
    {
      ...
      for (int index = size - 1; index >= 0; --index)
      {
        ...
        if (index == 0)
        {
          tmp = (mapContainer[index].matchData (tmp, dataContainer, covMatrix,
                                                5));
        }
        else
        {
