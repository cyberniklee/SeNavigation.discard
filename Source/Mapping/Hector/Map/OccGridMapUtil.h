...
namespace NS_HectorMapping
{
  ...
  template<typename ConcreteOccGridMap, typename ConcreteCacheMethod>
    ...
    public:
      ...
      getWorldCoordsPose (const Eigen::Vector3f& mapPose) const
      {
        return concreteGridMap->getWorldCoordsPose (mapPose);
      }
      ;
      inline Eigen::Vector3f
      getMapCoordsPose (const Eigen::Vector3f& worldPose) const
      {
      ...
      getWorldCoordsPoint (const Eigen::Vector2f& mapPoint) const
      {
      ...
      void
      getCompleteHessianDerivs (const Eigen::Vector3f& pose,
      ...
      Eigen::Matrix3f
      getCovarianceForPose (const Eigen::Vector3f& mapPose,
                            const DataContainer& dataPoints)
      {
      ...
      Eigen::Matrix3f
      getCovMatrixWorldCoords (const Eigen::Matrix3f& covMatMap)
      {
      ...
      float
      getLikelihoodForState (const Eigen::Vector3f& state,
                             const DataContainer& dataPoints)
      {
      ...
      float
      getLikelihoodForResidual (float residual, int numDataPoints)
      {
      ...
      float
      getResidualForState (const Eigen::Vector3f& state,
                           const DataContainer& dataPoints)
      {
        int size = dataPoints.getSize ();
      ...
      float
      getUnfilteredGridPoint (Eigen::Vector2i& gridCoords) const
      {
      ...
      float
      getUnfilteredGridPoint (int index) const
      {
        return (concreteGridMap->getGridProbabilityMap (index));
      }
      
      float
      interpMapValue (const Eigen::Vector2f& coords)
      {
      ...
      Eigen::Vector3f
      interpMapValueWithDerivatives (const Eigen::Vector2f& coords)
      {
        //check if coords are within map limits.
        if (concreteGridMap->pointOutOfMapBounds (coords))
        {
      ...
      Eigen::Affine2f
      getTransformForState (const Eigen::Vector3f& transVector) const
      {
      ...
      Eigen::Translation2f
      getTranslationForState (const Eigen::Vector3f& transVector) const
      {
        return Eigen::Translation2f (transVector[0], transVector[1]);
      }
