...
namespace NS_Planner
{
  ...
  SimpleTrajectoryGenerator::initialise (
      ...
      std::vector<Eigen::Vector3f> additional_samples, bool discretize_by_time)
  {
  ...
  void
  SimpleTrajectoryGenerator::initialise (const Eigen::Vector3f& pos,
                                         ...
                                         bool discretize_by_time)
  {
    ...
    if (vsamples[0] * vsamples[1] * vsamples[2] > 0)
    {
      ...
      if (!use_dwa_)
      {
        ...
        max_vel_x = std::max (std::min (max_vel_x, dist / sim_time_),
                              min_vel_x);
        max_vel_y = std::max (std::min (max_vel_y, dist / sim_time_),
                              min_vel_y);
  ...
  void
  SimpleTrajectoryGenerator::setParameters (double sim_time,
   ...
   */
  bool
  SimpleTrajectoryGenerator::hasMoreTrajectories ()
  {
   ...
   */
  bool
  SimpleTrajectoryGenerator::nextTrajectory (Trajectory &comp_traj)
  {
    bool result = false;
      ...
      Eigen::Vector3f sample_target_vel, NS_Planner::Trajectory& traj)
  {
    ...
    if ((limits_->min_trans_vel >= 0 && vmag + eps < limits_->min_trans_vel)
        && (limits_->min_rot_vel >= 0
            && fabs (sample_target_vel[2]) + eps < limits_->min_rot_vel))
    {
      return false;
    }
  ...
  Eigen::Vector3f
  SimpleTrajectoryGenerator::computeNewPositions (const Eigen::Vector3f& pos,
                                                  const Eigen::Vector3f& vel,
                                                  double dt)
  {
    Eigen::Vector3f new_pos = Eigen::Vector3f::Zero ();
    new_pos[0] = pos[0]
        + (vel[0] * cos (pos[2]) + vel[1] * cos (M_PI_2 + pos[2])) * dt;
    new_pos[1] = pos[1]
        + (vel[0] * sin (pos[2]) + vel[1] * sin (M_PI_2 + pos[2])) * dt;
   ...
   */
  Eigen::Vector3f
  SimpleTrajectoryGenerator::computeNewVelocities (
