...
namespace NS_Planner {
  ...
  DWAPlanner::DWAPlanner(NS_Planner::LocalPlannerUtil *planner_util, bool sum_scores, double cheat_factor,
                         double sim_time, double sim_granularity, double angular_sim_granularity,
                        double pdist_scale, double gdist_scale, double occdist_scale,
                        double stop_time_buffer, double oscillation_reset_dist, double oscillation_reset_angle,
      ...
      alignment_costs_(planner_util->getCostmap())
  {
    ...
    scored_sampling_planner_ = NS_Planner::SimpleScoredSamplingPlanner(generator_list, critics);

    cheat_factor_ = cheat_factor;

    generator_.setParameters(sim_time, sim_granularity, angular_sim_granularity, dwa, sim_period_);
    ...
    oscillation_costs_.setOscillationResetDist(oscillation_reset_dist, oscillation_reset_angle);
  ...
  bool DWAPlanner::setPlan(const std::vector<NS_DataType::PoseStamped>& orig_global_plan) {
    oscillation_costs_.resetOscillationFlags();
  ...
  bool DWAPlanner::checkTrajectory(
    ...
    Eigen::Vector3f goal(goal_pose.pose.position.x, goal_pose.pose.position.y, NS_Transform::getYaw(goal_pose.pose.orientation));
    ...
    NS_NaviCommon::console.warning("Invalid Trajectory %f, %f, %f, cost: %f", vel_samples[0], vel_samples[1], vel_samples[2], cost);
  ...
  NS_Planner::Trajectory DWAPlanner::findBestPath(
    ...
    if (result_traj_.cost_ < 0) {
      ...
      matrix.setRotation(NS_Transform::createQuaternionFromYaw(result_traj_.thetav_));
