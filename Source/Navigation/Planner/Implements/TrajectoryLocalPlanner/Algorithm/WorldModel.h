...
namespace NS_Planner {
  ...
  class WorldModel{
    public:
      ...
      double footprintCost(double x, double y, double theta, const std::vector<NS_DataType::Point>& footprint_spec, double inscribed_radius = 0.0, double circumscribed_radius=0.0){
        ...
        if(inscribed_radius==0.0){
          NS_CostMap::calculateMinAndMaxDistances(footprint_spec, inscribed_radius, circumscribed_radius);
        }
       ...
       * @return Positive if all the points lie outside the footprint, negative otherwise
       */
      double footprintCost(const NS_DataType::Point& position, const std::vector<NS_DataType::Point>& footprint,
