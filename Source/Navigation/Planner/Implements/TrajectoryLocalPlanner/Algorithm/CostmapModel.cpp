...
namespace NS_Planner
{
  ...
  double
  CostmapModel::footprintCost (const NS_DataType::Point& position,
                               ...
                               double circumscribed_radius)
  {
    ...
    //get the cell coord of the first point
    if (!costmap_.worldToMap (footprint.front ().x, footprint.front ().y, x1,
                              y1))
      return -1.0;
  ...
  double
  CostmapModel::lineCost (int x0, int x1, int y0, int y1)
  {
  ...
  double
  CostmapModel::pointCost (int x, int y)
  {
