...
namespace NS_Planner {
    ...
    if (! isInitialized()) {
      ...
      */
      ...
      if(meter_scoring) {
        ...
        NS_NaviCommon::console.warning("Trajectory Rollout planner initialized with param meter_scoring set to false. Set it to true to make your settins robust against changes of costmap resolution.");
      }
      ...
      min_in_place_vel_th_ = parameter.getParameter("min_in_place_rotational_vel", 0.4f);
      reached_goal_ = false;
      backup_vel = -0.1;
      backup_vel = parameter.getParameter("escape_vel", -0.1f);

      if(backup_vel >= 0.0)
        NS_NaviCommon::console.warning("You've specified a positive escape velocity. This is probably not what you want and will cause the robot to move forward instead of backward. You should probably change your escape_vel parameter to be negative");
      ...
      heading_scoring_timestep = parameter.getParameter("heading_scoring_timestep", 0.8f);
  ...
  bool TrajectoryLocalPlanner::rotateToGoal(const NS_Transform::Stamped<NS_Transform::Pose>& global_pose, const NS_Transform::Stamped<NS_Transform::Pose>& robot_vel, double goal_th, NS_DataType::Twist& cmd_vel){
    double yaw = NS_Transform::getYaw(global_pose.getRotation());
    double vel_yaw = NS_Transform::getYaw(robot_vel.getRotation());
    cmd_vel.linear.x = 0;
    cmd_vel.linear.y = 0;
  ...
  bool TrajectoryLocalPlanner::setPlan(const std::vector<NS_DataType::PoseStamped>& orig_global_plan){
    if (! isInitialized()) {
  ...
  bool TrajectoryLocalPlanner::computeVelocityCommands(NS_DataType::Twist& cmd_vel){
    if (! isInitialized()) {
      NS_NaviCommon::console.error("This planner has not been initialized, please call initialize() before using this planner");
      return false;
    }
    ...
    if (!transformGlobalPlan(service, global_plan_, global_pose, *costmap_, transformed_plan)) {
    ...
    */
    ...
    if (path.cost_ < 0) {
      NS_NaviCommon::console.debug("The rollout planner failed to find a valid plan. This means that the footprint of the robot was in collision for all simulated trajectories.");
      local_plan.clear();
      return false;
    }
  ...
  bool TrajectoryLocalPlanner::checkTrajectory(double vx_samp, double vy_samp, double vtheta_samp, bool update_map){
    ...
    if(costmap->getRobotPose(global_pose)){
      ...
      return tc_->checkTrajectory(global_pose.getOrigin().x(), global_pose.getOrigin().y(), NS_Transform::getYaw(global_pose.getRotation()),
          base_odom.twist.linear.x,
          base_odom.twist.linear.y,
          base_odom.twist.angular.z, vx_samp, vy_samp, vtheta_samp);

    }
    NS_NaviCommon::console.warning("Failed to get the pose of the robot. No trajectories will pass as legal in this case.");
    return false;
  }
  ...
  double TrajectoryLocalPlanner::scoreTrajectory(double vx_samp, double vy_samp, double vtheta_samp, bool update_map){
    ...
    if(costmap->getRobotPose(global_pose)){
      ...
      return tc_->scoreTrajectory(global_pose.getOrigin().x(), global_pose.getOrigin().y(), NS_Transform::getYaw(global_pose.getRotation()),
          base_odom.twist.linear.x,
          base_odom.twist.linear.y,
          base_odom.twist.angular.z, vx_samp, vy_samp, vtheta_samp);
