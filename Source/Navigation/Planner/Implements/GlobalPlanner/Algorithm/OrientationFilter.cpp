...
namespace NS_Planner
{
  ...
  void
  set_angle (NS_DataType::PoseStamped* pose, double angle)
  {
  ...
  double
  getYaw (NS_DataType::PoseStamped pose)
  {
  ...
  void
  OrientationFilter::processPath (const NS_DataType::PoseStamped& start,
  ...
  void
  OrientationFilter::pointToNext (std::vector<NS_DataType::PoseStamped>& path,
                                  int index)
  {
    double x0 = path[index].pose.position.x, y0 = path[index].pose.position.y,
        x1 = path[index + 1].pose.position.x, y1 =
  ...
  void
  OrientationFilter::interpolate (std::vector<NS_DataType::PoseStamped>& path,
