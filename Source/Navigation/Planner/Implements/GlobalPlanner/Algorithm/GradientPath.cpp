...
namespace NS_Planner
{
  ...
  void
  GradientPath::setSize (int xs, int ys)
  {
  ...
  bool
  GradientPath::getPath (float* potential, double start_x, double start_y,
                         ...
                         std::vector<std::pair<float, float> >& path)
  {
    ...
    while (c++ < ns * 4)
    {
      ...
      if (potential[stc] >= POT_HIGH || potential[stc + 1] >= POT_HIGH
          || potential[stc - 1] >= POT_HIGH || potential[stcnx] >= POT_HIGH
          || potential[stcnx + 1] >= POT_HIGH
          || potential[stcnx - 1] >= POT_HIGH || potential[stcpx] >= POT_HIGH
