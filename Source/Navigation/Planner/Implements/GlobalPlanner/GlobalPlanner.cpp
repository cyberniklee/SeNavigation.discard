...
//		NS_NaviCommon::console.debug("onInitialize running...");
		 ...
		 */
		unsigned int cx = costmap -> getLayeredCostmap() -> getCostmap() -> getSizeInCellsX(),
...
//	NS_NaviCommon::console.debug("After locking...");
	...
	if (!initialized_) {
		NS_NaviCommon::console.error("This planner has not been initialized yet, but it is being used, please call initialize() before use");
		return false;
	}
...
//	cout << "start world wx = " << wx << " wy = " << wy << "\n";
	 ...
	 */
	if(!costmap -> getLayeredCostmap() -> getCostmap() -> worldToMap(wx, wy, start_x_i, start_y_i)) {
		// 加一下错误提示
		NS_NaviCommon::console.error("The robot's start position is off the global costmap. Planning will always fail, are you sure the robot has been properly localized?");
		return false;
	}
...
//	cout << "goal world wx = " << wx << " wy = " << wy << "\n";
	...
	if(!costmap -> getLayeredCostmap() -> getCostmap() -> worldToMap(wx, wy, goal_x_i, goal_y_i)) {
		// 加一下错误提示
		NS_NaviCommon::console.error("The goal sent to the global planner is off the global costmap. Planning will always fail to this goal.");
		return false;
	}
...
//	NS_NaviCommon::console.debug("After parameters setSize...");
	...
	outlineMap(costmap -> getLayeredCostmap() -> getCostmap() -> getCharMap(),
			nx, ny, NS_CostMap::LETHAL_OBSTACLE);

//	NS_NaviCommon::console.debug("After outlineMap, invoking calculatePotentials...");
	/*
	 * 此处开始调用算法
	 */
	bool found_legal = planner_->calculatePotentials(costmap -> getLayeredCostmap() -> getCostmap() -> getCharMap(),
...
//	NS_NaviCommon::console.debug("After calculatePotentials, invoking clearEndPoint...");
	...
	planner_->clearEndpoint(costmap -> getLayeredCostmap() -> getCostmap() -> getCharMap(),
...
//	NS_NaviCommon::console.debug("After clearEndpoint...");
	...
	if (found_legal) {
		...
		if (getPlanFromPotential(start_x, start_y, goal_x, goal_y, goal, plan)) {
			...
			// 错误提示
			NS_NaviCommon::console.error("Failed to get a plan from potential when a legal potential was found. This shouldn't happen.");
		}
	}else{
		// 错误提示
...
void GlobalPlanner::clearRobotCell(unsigned int mx, unsigned int my) {
    ...
    costmap -> getLayeredCostmap() -> getCostmap() ->setCost(mx, my, NS_CostMap::FREE_SPACE);
}
...
void GlobalPlanner::mapToWorld(double mx, double my, double& wx, double& wy) {
    wx = costmap -> getLayeredCostmap() -> getCostmap() ->getOriginX()
    		+ (mx+convert_offset_) * costmap -> getLayeredCostmap() -> getCostmap() ->getResolution();
    wy = costmap -> getLayeredCostmap() -> getCostmap() ->getOriginY()
    		+ (my+convert_offset_) * costmap -> getLayeredCostmap() -> getCostmap() ->getResolution();
}
...
bool GlobalPlanner::worldToMap(double wx, double wy, double& mx, double& my) {
    ...
    if (mx < costmap -> getLayeredCostmap() -> getCostmap() ->getSizeInCellsX()
    		&& my < costmap -> getLayeredCostmap() -> getCostmap() ->getSizeInCellsY())
        return true;
