...
namespace NS_Controller{
  ...
  void SimulateController::setBaseTicksPerMeter(double ticksPerMeter){
  ...
  void SimulateController::setBaseWheelTrack(double wheelTrack){
  ...
  void SimulateController::setBaseWheelDiameter(double wheelDiameter){
    fBaseWheelDiameter = wheelDiameter;
  }

  void SimulateController::setBaseEncoderResolution(double encoderResolution){
    fBaseEncoderResolution = encoderResolution;
  }

  void SimulateController::setBaseGearReduction(double gearReduction){
    fBaseGearReduction = gearReduction;
  }

  void SimulateController::setAccelLimit(double accelLimit){
    fAccelLimit = accelLimit;
  }

  void SimulateController::setLinearVel(double linearVel){
    tNewVel.fLinear = linearVel;
  }

  void SimulateController::setAngularVel(double angularVel){
    tNewVel.fAngular = angularVel;
  }

  double SimulateController::getLinearVel(void){
    return tCurrentOdometry.linearVel;
  }

  double SimulateController::getAngularVel(void){
    return tCurrentOdometry.angularVel;
  }

  double SimulateController::getX(void){
    return tCurrentOdometry.fX;
  }

  double SimulateController::getY(void){
    return tCurrentOdometry.fY;
  }

  double SimulateController::getTheta(void){
    return tCurrentOdometry.fTheta;
  }

  void SimulateController::SeUserMotionCalcRollSpeed(SeUserVelocity tVel, short *tLeftSpeed, short *tRightSpeed){
  ...
  SeUserOdometry SimulateController::SeUserOdometryCalculate(int nLeftEncoderDelta, int nRightEncoderDelta, unsigned int nDeltaTime){
  ...
  void SimulateController::controllerPolling(void){
    ...
    {
      tCurOdom = SeUserOdometryCalculate(nLeftCount, nRightCount, nCntlDuration);
    }

    //NS_NaviCommon::console.message ("X=%f,Y=%f,Theta=%f,linearVel=%f,angularVel=%f", getX(),getY(),getTheta(),getLinearVel(),getAngularVel());
    SeUserMotionCalcRollSpeed(tCurVel, (short *)&fLeftDestVel, (short *)&fRightDestVel);
/*
...
*/
  ...
  void SimulateController::motionCallback(void){
  ...
  void SimulateController::initialize (){
  ...
  SimulateController::run (){
    ...
    simulate_controller_thread = boost::thread (boost::bind (&SimulateController::motionCallback, this));
